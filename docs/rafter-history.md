...
> **2026-06-07 KV-read clarification (RFT-83):** Direct read of `client:448e12a8-…` after deactivation of the legacy Make scenarios shows the orphan is a **hybrid record**, not pure dev values. **Andy's branded business identity is present**: `company_name` "2 Men and a Shovel", brand colours + fonts (`#0D2E1C` primary / `#84B741` accent / Playfair Display / Mulish), `phone` `(03) 9013 6588`, `business_address` Caulfield South, `abn` `18 652 417 171`, `business_email` `hello@2menandashovel.com`, `bank_details` (Andy's real BSB / account), all 24 templates, 16 credentials, 10 T&Cs, 6-tier payment_thresholds. **Infrastructure fields are dev/generic**: `webhook_url` = dev's `hook.eu1.make.com/i8gukma8...` (not Andy's prod `oh8gh9i7...`), `staff_uuid` = dev's `5ba57e76-…` (not Andy's `fe62e877-…`), `logo_url` = generic `/brand/rafter-logo.png`, `email_template` references the generic logo, `job_categories` and `job_queues` carry placeholder strings (`Standard / VIP / Warranty / After-Hours`, `Workshop / Pending Quotes / Parts on Order`) rather than Andy's actual values. The record also retains the deprecated `r2_photo_path` field (dropped from modern schema in commit `3ab5de2`) and the old `proposal_types` string-array shape, dating its last write to pre-RFT-53 code. The "real materials (117 items)" and "real customer list" mentioned in the consequence paragraph above refer to SM8 vendor-side data, not KV record content — materials live in `materials:{uuid}` cache, customers live in SM8. **Reads:** nothing reads this record today — `slug:andy → 0e604a45`, no `clerk_org:*` entry points here, all worker code paths resolve by slug or Clerk org rather than raw UUID. **Writes:** last refreshed by the legacy Make TOKEN_WEBHOOK (scenario 5612449) before its deactivation on 2026-06-07; with both legacy scenarios off, no writer remained. **KV record deleted 2026-06-07 after operator sign-off.** The UUID `448e12a8-…` itself remains safety-flagged per the CLAUDE.md safety table; never create another client record at this key.
