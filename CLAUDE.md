...
| BUG-18 | Email subject hardcodes "2 Men and a Shovel" + mojibake en-dash + unwanted quote ref | **Closed** | P2 | Code | Rafter Form module 37 subject was `"Your quote from 2 Men and a Shovel â€" {{1.quote_ref}}"` — hardcoded name, literal mojibake bytes (UTF-8 en-dash misinterpreted as Windows-1252 and re-saved), and an unwanted quote ref. Patched via Make API to `"Your quote from {{35.data.company_name}}"`. Dev scenario (5761732) not touched. Email body in KV `email_template` for both trial and live also updated: removed the `<p><strong>Quote reference:</strong> {quote_ref}<br><strong>Total (inc. GST):</strong> ${total}</p>` block. Verified via `/render-email`. |
