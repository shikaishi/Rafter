...
| BUG-24 | Make scenario auto-disabled on transient errors (Option B hardening) | **Closed** | P1 | Code | Make's default `maxErrors=3` disabled the entire scenario after 3 consecutive errors of any kind — fragile for a long-lived integration with transient SM8/Gmail failures. Fix applied to prod scenario (5537814) via Make API: `maxErrors: 3 → 999`, `dlq: false → true` (incomplete executions preserved), plus per-module error handlers — Break (retry 3× at 5min) on M35, M2, M3, M14, M15 (must-succeed modules); Resume (continue with empty fallback) on M13, M33, M12, M17 (recoverable / nice-to-have modules). Pre- and post-change blueprints committed in `make-blueprints/`. Verified by firing 3 deliberately-bad webhooks: scenario stayed `isActive=true`, `dlqCount` went 0→3. Test entries in DLQ tagged with `_test_marker: option-b-verification-2026-05-21` for manual deletion via Make UI. Trial scenario (5761732) not touched. |
